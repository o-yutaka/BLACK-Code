...
function Test-Model([string]$Path){
    if(-not(Test-Path $Path)){return $false};$item=Get-Item $Path;if($item.Length -lt $ModelMinimumBytes){return $false}
    $stream=[System.IO.File]::OpenRead($Path);try{$bytes=New-Object byte[] 4;[void]$stream.Read($bytes,0,4);if([System.Text.Encoding]::ASCII.GetString($bytes)-ne "GGUF"){return $false}}finally{$stream.Dispose()}
