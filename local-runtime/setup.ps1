...
function Test-Model([string]$Path) {
    ...
    $item = Get-Item $Path
    if ($item.Length -lt 10000000000) { return $false }
...
Write-Step "Checking curl and OpenCode"
...
if ($Force -or $ForceLlama -or -not (Test-Path $ServerExe)) {
    ...
    Download-File $picked.main.browser_download_url $mainZip
    Download-File $picked.cudart.browser_download_url $cudaZip
...
# Canonical runtime is IQ2_M only. Remove the superseded IQ4_XS local weight after IQ2_M is verified.
...
$launcherFiles = @("black-code.ps1", "setup.ps1", "doctor.ps1", "execution-fabric.ps1", "black-code-execution.md", "verify.ps1")
...
$state = [ordered]@{
    ...
    gpu = ($gpuInfo -join "; ")
    execution_fabric = "black-execution-fabric-iq2m-fast-v1"
...
Write-Host ""
Write-Host "BLACK CODE IQ2_M FAST RUNTIME VERIFIED" -ForegroundColor Green
