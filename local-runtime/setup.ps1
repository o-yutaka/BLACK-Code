...
function Download-File([string]$Url, [string]$Destination) {
    ...
    $curlArgs = @("-L", "--fail", "--retry", "8", "--retry-all-errors", "--retry-delay", "2", "-o", $partial, $Url)
    if (Test-Path -LiteralPath $partial) { $curlArgs = @("-C", "-") + $curlArgs }
    ...
    try {
        ...
        $curlProcess = Start-Process -FilePath $curlPath -ArgumentList $curlArgs -Wait -PassThru `
            -RedirectStandardOutput $curlStdout -RedirectStandardError $curlStderr
    ...
    finally { Remove-Item -Force -ErrorAction SilentlyContinue $curlStdout, $curlStderr }
    if (-not (Test-Path -LiteralPath $partial)) { throw "curl completed without creating the partial download: $partial" }
...
function Test-Model([string]$Path) {
    ...
    try {
        $bytes = New-Object byte[] 4; [void]$stream.Read($bytes, 0, 4)
        if ([System.Text.Encoding]::ASCII.GetString($bytes) -ne "GGUF") { return $false }
    }
    finally { $stream.Dispose() }
    return (Get-FileHash -Algorithm SHA256 -Path $Path).Hash.ToLowerInvariant() -eq $ModelSha256
}
...
$gpuInfo = Invoke-NvidiaSmi "name,memory.total"; Write-Host $gpuInfo
...
if (-not (Get-Command "opencode" -ErrorAction SilentlyContinue)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "OpenCode installation failed with exit code $LASTEXITCODE" }
    Refresh-Path
}
...
if ($Force -or $ForceLlama -or -not (Test-Path $ServerExe)) {
    ...
    foreach ($release in $releases) {
        ...
        if ($main -and $cudart) { $picked = [PSCustomObject]@{ tag = $release.tag_name; main = $main; cudart = $cudart }; break }
    }
    if (-not $picked) { throw "Could not find a recent llama.cpp Windows CUDA 12.4 release." }
...
if (-not (Test-Model $ModelPath)) { throw "Model verification failed. Expected SHA256 $ModelSha256" }
...
foreach ($name in $launcherFiles) {
    ...
    if (Test-Path $source) { Copy-Item -Force $source (Join-Path $LauncherDir $name) }
}
...
Write-Host ""; Write-Host "    black-code" -ForegroundColor Yellow
